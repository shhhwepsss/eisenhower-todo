...
describe('createTask', () => {
  it('рождает неразобранную задачу во «Входящих» (MANUAL_PRIORITISATION)', () => {
    const task: Task = createTask({ id: 'task-1', title: 'купить хлеб', now: NOW });
  ...
  it('ID_IS_GLOBALLY_UNIQUE: id приходит снаружи, домен его не конструирует', () => {
    const id: string = '0f6d5a2e-8f7c-4c2a-9a1e-6c0b8f2d4e11';
  ...
  it('заголовок и описание обрезаются по краям', () => {
    const task: Task = createTask({ id: 'task-1', title: '  дело  ', text: '  подробности  ', now: NOW });
