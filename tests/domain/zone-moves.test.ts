...
describe('ZONE_MOVES', () => {
  ...
  it('каждая строка совпадает с тем, что moveToZone делает на самом деле', () => {
    for (const from of ZONES) {
      for (const to of ZONES) {
        ...
        const moved = moveToZone(task, placementOfZone(to), between, LATER);

        expect(zoneOf(placementOf(moved)), label).toBe(to);
  ...
  it('переход внутри «Входящих» вообще ничего не меняет', () => {
    ...
    expect(moveToZone(task, placementOfZone('inbox'), { before: null, after: null }, LATER)).toBe(
      task,
    );
  });
});
