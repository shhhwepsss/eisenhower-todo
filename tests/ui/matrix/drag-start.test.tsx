 ...
 * второму — чтобы `isFromControl` перестал отсеивать контролы. Оба следят за
...
describe('захват карточки', () => {
  ...
  it('нажатие на переключатель разбора жест не начинает', async () => {
    await renderWithStore(<MatrixTab />, { stored: [createTask({ id: 'a', title: 'разобрать', now: NOW })] });

    grabAndMove(within(card('разобрать')).getByRole('button', { name: 'Срочная' }));
